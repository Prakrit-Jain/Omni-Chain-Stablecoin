...
interface ISPRTStaking {
	...
	event TotalSPRTStakedUpdated(uint256 _totalSPRTStaked);
	...
	function unstake(uint256 _SPRTamount) external;

	function increaseFee_Asset(address _asset, uint256 _AssetFee) external;

	function increaseFee_DebtToken(uint256 _SPRTFee) external;
