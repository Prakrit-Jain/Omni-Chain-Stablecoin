...
contract FeeCollector is IFeeCollector, UUPSUpgradeable, OwnableUpgradeable, Addresses {
	...
	bool public constant routeToSPRTStaking = false; // if true, collected fees go to stakers; if false, to the treasury
	...
	function handleRedemptionFee(address _asset, uint256 _amount) external onlyVesselManager {
		if (_routeToSPRTStaking()) {
			ISPRTStaking(sprtStaking).increaseFee_Asset(_asset, _amount);
		}
		emit RedemptionFeeCollected(_asset, _amount);
	}

	function getProtocolRevenueDestination() public view override returns (address) {
		return _routeToSPRTStaking() ? sprtStaking : treasuryAddress;
	}
	...
	/**
	 * Transfers collected (debt token) fees to either the treasury or the SPRTStaking contract, depending on a flag.
	 */
	function _collectFee(address _borrower, address _asset, uint256 _feeAmount) internal {
		if (_feeAmount != 0) {
			...
			if (_routeToSPRTStaking()) {
				ISPRTStaking(sprtStaking).increaseFee_DebtToken(_feeAmount);
			}
	 ...
	 */
	function _routeToSPRTStaking() internal view virtual returns (bool) {
		return routeToSPRTStaking;
	}
