...
contract("Gas cost tests", async accounts => {
    ...
    */
	...
	it("", async () => {
		...
		// >>FF time and one account tops up, triggers SPRT gains for all
		await th.fastForwardTime(timeValues.SECONDS_IN_ONE_HOUR, web3.currentProvider)
		await stabilityPool.provideToSP(dec(1, 18), ZERO_ADDRESS, { from: _10_Accounts[0] })

		// Check the other accounts have SPRT gain
		for (account of _10_Accounts.slice(1)) {
			const SPRTGain = await stabilityPool.getDepositorSPRTGain(account)
			assert.isTrue(SPRTGain.gt(toBN("0")))
		}
	...
	//   // Check accounts have SPRT gains from liquidations
