...
"""# SPRT price
In the first month, the price of SPRT follows

> $P_t^q = P_{t-1}^q (1+\zeta_t^q)(1+\sigma_t^q)$. 

Note that $\zeta_t^q \sim N(0,$ sd_SPRT) represents SPRT price shock and $\sigma_t^q$ the drift. Here, $\sigma_t^q =$ drift_SPRT, so that the expected SPRT price increases from price_SPRT_initial to the following at the end of the first month:
> $E(P_{720}^q) = $price_SPRT_initial$ \cdot (1+$ drift_SPRT$)^{720}$

The SPRT price from the second month on is endogenously determined.
"""

# SPRT price & airdrop
price_SPRT_initial = 0.4
...
"""**SPRT Endogenous Price**

The staked SPRT pool earning consists of the issuance fee revenue and redemption fee revenue
> $R_t^q = R_t^i + R_t^r.$
...
> $$r_t=\frac{SPRT Market Cap}{Annualized Earning}=\frac{MC_t}{E_t}$$

> $MC_t=P_t^q \cdot$ SPRT_total_supply

Therefore, the SPRT price dynamics is determined
> $$P_t^q=discount \cdot \frac{r^{PE}}{SPRT\_total\_supply}E_t$$

Interpretation: The denominator implies that with more SPRT tokens issued, SPRT price decreases. However, the depreciation effect can be counteracted by the growth of the earning.
...
$R_{t-1}^s$ is the return in the stability pool, which consists of liquidation gain and airdrop SPRT gain.
...
- $R_t^a=P_{t}^q\hat{Q}_t^q$ is the airdrop gain, $\hat{Q}_t^q=1000$ denotes the amount of SPRT token airdropped to the stability pool providers
...
# F = 0.5 ** (1/8760)
# F = 0.99992087674
def quantity_SPRT_airdrop(index):
    F = 0.99992087674
    if index <= 0:
        return 0
...
def liquidate_vessels(
    ...
    airdrop_gain = price_SPRT_current * quantity_SPRT_airdrop(index)
...
def SPRT_market(index, data):
    # quantity_SPRT = (SPRT_total_supply/3)*(1-0.5**(index/period))
    ...
    return [price_SPRT_current, annualized_earning]
